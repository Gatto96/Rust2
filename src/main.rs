...
// - Create a `src/lib.rs` file --CHECK
// - Move all of the constants into lib.rs --CHECK
// - Make the constants public by adding the `pub` keyword in front of them--CHECK
...
//
// - In your lib.rs file add the line `pub mod sound;`--CHECK
// - Create a `src/sound.rs` file for your module --CHECK
// - Move the `dog`, `cat`, and `fox` functions into sound.rs --CHECK
// - Make the functions public by adding the `pub` keyword in front of them --CHECK
